...
import { Settings, BellRing, Cpu } from 'lucide-react';
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card title="Venue Metadata" subtitle="Core specifications for the active stadium profile">
            <div className="space-y-4 mt-4">
              <div>
                <label htmlFor="stadiumProfileName" className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block mb-1.5">Stadium Profile Name</label>
                <input 
                  id="stadiumProfileName"
                  type="text" 
                  value={venueName} 
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label htmlFor="maxSeatCapacity" className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block mb-1.5">Max Seat Capacity</label>
                  <input 
                    id="maxSeatCapacity"
                    type="number" 
                    value={capacity} 
                    onChange={(e) => setCapacity(Number(e.target.value))}
                    className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3.5 py-2 text-xs text-slate-300 focus:outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/50 font-mono"
                    required
                  />
                </div>
                <div>
                  <label htmlFor="hostCityLocation" className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block mb-1.5">Host City Location</label>
                  <input 
                    id="hostCityLocation"
                    type="text" 
                    value="Metropolis, US" 
                    className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3.5 py-2 text-xs text-slate-400 cursor-not-allowed focus:outline-none"
                    disabled
                  />
                </div>
              </div>
          ...
          <Card title="Telemetry Thresholds" subtitle="Configure system alert levels for queue lengths and sensor status">
            <div className="space-y-4 mt-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label htmlFor="queueWarning" className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block mb-1.5">Queue Warning (mins)</label>
                  <input 
                    id="queueWarning"
                    type="number" 
                    value={queueWarning} 
                ...
                <div>
                  <label htmlFor="queueDanger" className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block mb-1.5">Queue Danger (mins)</label>
                  <input 
                    id="queueDanger"
                    type="number" 
                    value={queueDanger} 
              ...
              <div>
                <label htmlFor="safetyIndexThreshold" className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block mb-1.5">Safety Index Threshold</label>
                <select 
                  id="safetyIndexThreshold"
                  className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3.5 py-2 text-xs text-slate-300 focus:outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/50 cursor-pointer"
                >
          ...
          <Card title="Decision Engine Parameters" subtitle="Tweak cognitive reasoning limits and dispatcher models">
            <div className="space-y-4 mt-4">
              <div>
                <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block mb-1.5">Active Dispatcher Model</span>
              ...
              <div>
                <label htmlFor="minConfidenceThreshold" className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block mb-1.5">Minimum Confidence Threshold ({aiConfidence}%)</label>
                <input 
                  ...
                  className="w-full h-1.5 bg-slate-900 rounded-lg appearance-none cursor-pointer accent-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50"
                />
          ...
          <Card title="Perimeter Integrations" subtitle="Push notifications and system hooks endpoint settings">
            <div className="space-y-4 mt-4">
              <div>
                <label htmlFor="staffWebhook" className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block mb-1.5">Staff Mobile Dispatch Webhook</label>
                <input 
                  id="staffWebhook"
                  type="url" 
